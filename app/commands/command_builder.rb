...
class CommandBuilder
  ...
  def attrs
    Command::Attrs.new(chat_id:, chat_type:, user_id:, bot_api:, kronika_api:)
  end
  ...
  def location_sharing_command
    SendLocationSharingRequestCommand.new(attrs)
  end

  def help_message_command
    SendHelpMessageCommand.new(attrs)
  end

  def convert_time_command(time_str)
    ConvertTimeCommand.new(attrs:, time_str:)
  end

  def drop_timezone_command
    DropTimezoneCommand.new(attrs)
  end

  def read_timezone_command
    ReadTimezoneCommand.new(attrs)
  end

  def save_timezone_command(input)
    SaveTimezoneCommand.new(attrs:, input:)
  end
